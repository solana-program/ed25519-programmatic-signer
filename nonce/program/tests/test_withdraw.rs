...
fn advance_does_not_bypass_same_slot_close_guard() {
    ...
    let state: Nonce = wincode::deserialize_exact(&nonce.data).unwrap();
    let advance_instruction = advance(&authority, &nonce_address, state.nonce, Hash::default());
    let advanced = mollusk.process_and_validate_instruction(
        &advance_instruction,
        &[(authority, Account::default()), (nonce_address, nonce)],
        &[Check::success()],
    );
    WithdrawBuilder::default()
        .nonce_account((

...
pub fn install(mollusk: &mut Mollusk) {
    mollusk.add_program(&spl_legacy_message_executor_interface::id(), NAME);
}
...
pub fn wrap(instruction: Instruction) -> Instruction {
    ...
    Instruction {
        program_id: spl_legacy_message_executor_interface::id(),
        accounts,
        data,
    }
}
...
pub fn keyed_account() -> (Address, Account) {
    let program_id = spl_legacy_message_executor_interface::id();

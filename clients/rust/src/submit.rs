...
pub fn submit_transaction(
    ...
    let mut instruction = submit(transaction.signatures.clone(), transaction.message.clone());
    ...
    let bytes = wincode::serialize(&transaction).map_err(|_| Error::SerializeTransaction)?;
    if bytes.len() > 1232 {
        return Err(Error::TransactionTooLarge(bytes.len()));
    }
    Ok(transaction)
}

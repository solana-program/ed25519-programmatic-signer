...
pub fn verify_static(transaction: &VersionedTransaction) -> Result<()> {
    ...
    for (index, key) in message.account_keys.iter().enumerate() {
        if message.account_keys[index.saturating_add(1)..].contains(key) {
            return Err(Error::DuplicateMessageAccount(*key));
        }
    }
    let bytes = transaction.message.serialize();
    for (key, signature) in message.account_keys.iter().zip(&transaction.signatures) {
        if *signature != Signature::default() && !signature.verify(key.as_ref(), &bytes) {
            return Err(Error::InvalidSignature(*key));
        }
    }

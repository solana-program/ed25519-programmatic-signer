...
pub fn transaction_from_message(
    ...
    let VersionedMessage::Legacy(inner_message) = inner_message else {
        return Err(Error::UnsupportedInnerMessage);
    };
    ...
    let transaction = unsigned_transaction(message)?;
    crate::verify_static(&transaction)?;
    Ok(transaction)
}
...
pub fn merge_transactions(
    ...
    let mut merged = transaction.signatures.clone();
...
pub fn is_fully_signed(transaction: &VersionedTransaction) -> bool {
    crate::verify_static(transaction).is_ok()

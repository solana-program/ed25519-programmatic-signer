...
pub enum Error {
    ...
    NotFullySigned,
    /// The serialized relay transaction exceeds the network packet limit.
    ...
    InvalidInnerMessage,
    /// The current executor only accepts legacy inner messages.
    UnsupportedInnerMessage,
...
impl fmt::Display for Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TransactionTooLarge(bytes) => write!(
                formatter,
                "relay transaction is {bytes} bytes; maximum is 1232"
            ),
            ...
            Self::UnsupportedInnerMessage => {
                formatter.write_str("only legacy inner messages are supported")
            }

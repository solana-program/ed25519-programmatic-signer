...
#[test]
fn sign_rejects_duplicate_required_signer_slots() {
    let signer = Keypair::new();
    ...
    assert_eq!(
        sign_transaction(&mut transaction, &signer),
        Err(Error::DuplicateMessageAccount(signer.pubkey()))
    );
    assert!(
        transaction
            .signatures
            .iter()
            .all(|signature| *signature == solana_signature::Signature::default())
    );
}
...
fn verify_rejects_inner_account_order_mismatch() {
    ...
    message.instructions[0].accounts.swap(2, 3);
...
#[test]
fn construction_rejects_v0_and_v1_inner_messages() {
    let authority = Keypair::new().pubkey();
    let pda = ProgrammaticSigner::derive_address(&spl_ed25519_signer_interface::id(), &authority);
    ...
    for message in messages {
        assert_eq!(
            transaction_from_message(message, test_address(12), &[authority], &[], genesis_hash()),
            Err(Error::UnsupportedInnerMessage)
        );
    }
}
...
fn verify_rejects_missing_wrapped_writable_privilege() {
    ...
    assert_eq!(
        verify(&transaction, &nonce, &nonce_account, &genesis_hash()),
        Err(Error::InvalidNonceAccountMeta)
    );
}
...
fn transaction_for_inner_message(
    ...
    let VersionedMessage::Legacy(inner_message) = inner_message else {
        panic!("legacy fixture required")
    };
...
#[test]
fn submit_rejects_oversized_relay_before_sending() {
    let authority = Keypair::new();
    let plan = TransactionPlan::new(
        vec![Instruction {
            program_id: test_address(60),
            accounts: vec![],
            data: vec![0; 1232],
        }],
    ...
    .unwrap();
    let mut transaction = build_transaction(&plan, Hash::default(), genesis_hash()).unwrap();
    sign_transaction(&mut transaction, &authority).unwrap();
    assert!(matches!(
        submit_transaction(&transaction, &Keypair::new(), &[], Hash::default()),
        Err(Error::TransactionTooLarge(_))
    ));
}

#[test]
fn malformed_empty_transaction_is_not_fully_signed() {
    assert!(!is_fully_signed(&VersionedTransaction {
        signatures: vec![],

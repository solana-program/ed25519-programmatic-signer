...
use {
    ...
    spl_legacy_message_executor_interface::error::Error as MessageExecutorError,
    spl_nonce_client::instruction::{advance, create_account as create_nonce_account_instructions},
    spl_nonce_interface::state::Nonce,
    spl_programmatic_signer_client::{
        Error, TransactionPlan, build_transaction, nonce::next_nonce, sign_transaction,
        submit::submit_transaction, verify::verify,
    },
};
...
fn cancel_pda_authority() {
    ...
    let expected_next_nonce = next_nonce(&cancellation).unwrap();
    assert_eq!(
...
#[test]
fn precomputed_chain_requires_order_and_matches_predicted_nonces() {
    let harness = Harness::new();
    let authority = Keypair::new();
    let pda = programmatic_signer(&authority);
    let first_recipient = Address::new_unique();
    ...
    fund_transfer_accounts(&harness, pda, first_recipient);
    store_account(&harness.context, second_recipient, system_account(0));
    let first = harness.signed_transfer(nonce_account, &authority, first_recipient);
    let after_first = next_nonce(&first).unwrap();
    let plan = TransactionPlan::transfer(
        nonce_account,
        authority.pubkey(),
        second_recipient,
        TRANSFER_LAMPORTS,
    ...
    .unwrap();
    let mut second = build_transaction(&plan, after_first, genesis_hash()).unwrap();
    sign_transaction(&mut second, &authority).unwrap();
    let after_second = next_nonce(&second).unwrap();
    let first_relay = harness.submit(&first, test_hash(30));
    let second_relay = harness.submit(&second, test_hash(31));
    expect_program_failure(
        &harness.context,
        &second_relay,
        spl_legacy_message_executor_interface::id(),
    ...
    harness.process_success(&first_relay);
    assert_eq!(
        harness.nonce_account_state(&nonce_account).nonce,
        after_first
    );
    harness.process_success(&second_relay);
    assert_eq!(
        harness.nonce_account_state(&nonce_account).nonce,
        after_second
    );
...
#[test]
fn failed_inner_transfer_preserves_nonce_and_balances() {
    let harness = Harness::new();
    let authority = Keypair::new();
    let pda = programmatic_signer(&authority);
    let recipient = Address::new_unique();
    let nonce_account = harness.create_nonce_account(&pda);
    fund_transfer_accounts(&harness, pda, recipient);
    let state = harness.nonce_account_state(&nonce_account);
    let plan = TransactionPlan::transfer(
        nonce_account,
        authority.pubkey(),
        recipient,
        PDA_LAMPORTS.saturating_add(1),
    )
    .unwrap();
    let mut transaction = build_transaction(&plan, state.nonce, genesis_hash()).unwrap();
    sign_transaction(&mut transaction, &authority).unwrap();
    let relay = harness.submit(&transaction, test_hash(32));
    let result = process_outer_transaction(&harness.context, &relay, &[]);
    assert!(matches!(result.program_result, ProgramResult::Failure(_)));
    assert_eq!(harness.nonce_account_state(&nonce_account), state);
    assert_eq!(account(&harness.context, &pda).lamports, PDA_LAMPORTS);
    assert_eq!(account(&harness.context, &recipient).lamports, 0);
}

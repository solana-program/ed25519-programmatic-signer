...
pub async fn run(command: SubmitCommand, client: &Client, output: OutputFormat) -> Result<String> {
    ...
    // Wrap the approved execution message in a `Submit` relay transaction and broadcast it
    let instruction =
...
fn collect_signatures(
    ...
    let mut signatures_by_authority = file_signatures;
